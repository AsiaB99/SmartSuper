...
@section('content')
            ...
            <section class="mb-12 rounded-[20px] bg-white p-10 text-center shadow-[0_10px_30px_rgba(0,0,0,0.05)]">
                ...
                <div class="mt-6 flex justify-center">
                    <a class="ss-btn-green" href="{{ route('despensas.create') }}">
                        ...
                        <span>{{ __('despensas.index.create') }}</span>
                    </a>
                </div>
            </section>
    ...
    <script>
        (() => {
            const translations = {
                deleteText: @json(__('despensas.index.delete_text', ['name' => '__PANTRY__'])),
                removeUserAria: @json(__('listas.edit.remove_user_aria', ['user' => '__USER__'])),
                invalidUsername: @json(__('listas.edit.errors.invalid_username')),
                duplicateUsername: @json(__('listas.edit.errors.duplicate_username')),
                loadData: @json(__('listas.edit.errors.load_data')),
                loadCollaborators: @json(__('despensas.edit.load_collaborators')),
            };
            const deleteDialog = document.getElementById('delete-despensa-dialog');
            const deleteDialogText = deleteDialog?.querySelector('p');
            const deleteDespensaNombre = document.getElementById('delete-despensa-nombre');
            const deleteConfirmButton = document.getElementById('delete-despensa-confirm');
            const deleteCancelButton = document.getElementById('delete-despensa-cancel');
            let currentDeleteForm = null;

            if (deleteDialog && deleteDespensaNombre && deleteConfirmButton && deleteCancelButton) {
                document.querySelectorAll('[data-delete-despensa]').forEach((button) => {
                    button.addEventListener('click', () => {
                        currentDeleteForm = button.closest('form');
                        deleteDespensaNombre.textContent = button.dataset.despensaNombre ?? '';
                        if (deleteDialogText) {
            ...
            const editDialog = document.getElementById('edit-despensa-dialog');
            const editForm = document.getElementById('edit-despensa-form');
            const editNombre = document.getElementById('edit-despensa-nombre');
            const editCancelButton = document.getElementById('edit-despensa-cancel');
            const editCloseButton = document.getElementById('edit-despensa-close');
            const editSubmitButton = document.getElementById('edit-despensa-submit');
            const usuariosSection = document.getElementById('edit-despensa-usuarios-section');
            const usuariosError = document.getElementById('edit-despensa-usuarios-error');
            const usuariosSelected = document.getElementById('edit-despensa-selected-users');
            const usuariosSearch = document.getElementById('edit-despensa-usuarios-search');
            const usuariosEmptyState = document.getElementById('edit-despensa-empty-state');
            const hiddenInputs = document.getElementById('edit-despensa-hidden-inputs');
            const editButtons = document.querySelectorAll('[data-edit-despensa]');
            ...
            const fillEditModal = (button, payload) => {
                ...
                if (editNombre) {
                    editNombre.value = payload?.despensa?.nombre_despensa ?? button.dataset.despensaNombre ?? '';
                }
            ...
            if (editDialog && editForm && editNombre && editCancelButton && editButtons.length > 0) {
                ...
                editButtons.forEach((button) => {
                    button.addEventListener('click', async () => {
                        resetUserPicker();
                        editForm.setAttribute('action', button.dataset.editUrl ?? '');
                        editNombre.value = button.dataset.despensaNombre ?? '';
