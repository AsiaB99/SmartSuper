...
@section('title', __('productos.edit.meta_title'))
...
@section('content')
    <section class="flex flex-wrap items-center justify-between gap-6 rounded-lg border border-white/70 bg-white/85 p-6 shadow-soft">
        <div>
            ...
            <h1 class="mt-2 font-display text-4xl text-ink-900">{{ __('productos.edit.title') }}</h1>
            <p class="mt-3 text-sm leading-7 text-ink-600">{{ __('productos.edit.subtitle') }}</p>
        </div>
    </section>

    <section class="mx-auto mt-6 max-w-3xl rounded-lg border border-white/70 bg-white/85 p-6 shadow-soft">
        <form action="{{ route('admin.productos.update', $producto) }}" method="POST" class="grid gap-5">
            @csrf
            @method('PUT')

            <fieldset class="grid gap-2">
                <label class="text-sm font-semibold text-ink-700" for="nombre_producto">{{ __('productos.form.name') }}</label>
                <input class="rounded-lg border border-brand-100 bg-white px-4 py-3 text-ink-900 focus:border-brand-400 focus:ring-brand-300" type="text" id="nombre_producto" name="nombre_producto" value="{{ old('nombre_producto', $producto->nombre_producto) }}" required>
            ...
            <fieldset class="grid gap-2">
                ...
                <input class="rounded-lg border border-brand-100 bg-white px-4 py-3 text-ink-900 focus:border-brand-400 focus:ring-brand-300" type="text" id="marca" name="marca" value="{{ old('marca', $producto->marca) }}">
                @error('marca')
                    <span class="text-sm font-medium text-rose-600">{{ $message }}</span>
                @enderror
            </fieldset>
            ...
            <fieldset class="grid gap-2">
                ...
                <input class="rounded-lg border border-brand-100 bg-white px-4 py-3 text-ink-900 focus:border-brand-400 focus:ring-brand-300" type="text" id="formato" name="formato" value="{{ old('formato', $producto->formato) }}">
                @error('formato')
                    <span class="text-sm font-medium text-rose-600">{{ $message }}</span>
                @enderror
            </fieldset>
            ...
            <fieldset class="grid gap-2">
                ...
                <input class="rounded-lg border border-brand-100 bg-white px-4 py-3 text-ink-900 focus:border-brand-400 focus:ring-brand-300" type="text" id="imagen" name="imagen" value="{{ old('imagen', $producto->imagen) }}">
                @error('imagen')
                    <span class="text-sm font-medium text-rose-600">{{ $message }}</span>
                @enderror
            </fieldset>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button type="submit" class="inline-flex items-center rounded-full bg-brand-600 px-5 py-3 text-sm font-semibold text-white shadow-soft transition hover:-translate-y-0.5 hover:bg-brand-700">{{ __('productos.edit.submit') }}</button>
