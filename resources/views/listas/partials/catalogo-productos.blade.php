            ...
            <article class="flex h-full flex-col rounded-[12px] border border-[var(--color-borde-suave)] bg-[var(--color-fondo-claro)] p-4 shadow-[0_3px_10px_rgba(0,0,0,0.04)]">
                <div class="flex items-start gap-3">
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-xs text-ink-500">{{ collect([$producto->marca, $producto->formato])->filter()->join(' · ') ?: 'Producto disponible en catálogo' }}</p>
                    </div>
                </div>
                ...
                <form class="mt-4 flex flex-1 flex-col justify-end gap-2" action="{{ route('listas.productos.agregar', $lista) }}" method="POST">
                    ...
                    @endif
                    <button class="ss-btn-green w-full justify-center px-4 py-2.5 text-sm" type="submit">Añadir</button>
                </form>
            </article>
        @empty
            <div class="rounded-[10px] border border-dashed border-brand-200 bg-white p-5 sm:col-span-2 lg:col-span-3">
                <h3 class="text-base font-semibold text-ink-900">Sin coincidencias en catálogo</h3>
                <p class="mt-2 text-sm leading-6 text-ink-600">Prueba otro término para encontrar productos.</p>
            </div>
        @endforelse
    </div>
