<div class="grid gap-5 md:grid-cols-2">
    ...
    <label class="grid gap-2">
        ...
        <select class="ss-input" name="estado" required>
            <option value="activa" @selected(old('estado', $lista->estado ?? 'activa') === 'activa')>{{ __('common.states.activa') }}</option>
            <option value="comprada" @selected(old('estado', $lista->estado ?? '') === 'comprada')>{{ __('common.states.comprada') }}</option>
        </select>
    ...
    @if (($mostrarFechaCreacion ?? true))
        <label class="grid gap-2">
            <span class="text-sm font-semibold text-ink-700">{{ __('listas.form.created_at') }}</span>
    ...
    @if (($puedeAsignarEditores ?? false))
        <label class="grid gap-2 md:col-span-2">
            ...
            <select class="ss-input min-h-40" name="usuarios_editores[]" multiple>
                ...
                @empty
                    <option value="" disabled>{{ __('listas.form.no_users') }}</option>
                @endforelse
            </select>
