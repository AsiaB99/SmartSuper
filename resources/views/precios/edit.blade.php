...
@section('title', __('precios.edit.meta_title'))
...
@section('content')
    <section class="flex flex-wrap items-center justify-between gap-6 rounded-lg border border-white/70 bg-white/85 p-6 shadow-soft">
        <div>
            ...
            <h1 class="mt-2 font-display text-4xl text-ink-900">{{ __('precios.edit.title') }}</h1>
            <p class="mt-3 text-sm leading-7 text-ink-600">{{ __('precios.edit.subtitle') }}</p>
        </div>
    </section>
    ...
    <section class="mx-auto mt-6 max-w-3xl rounded-lg border border-white/70 bg-white/85 p-6 shadow-soft">
        <form action="{{ route('admin.precios.update', [$precio->id_producto, $precio->id_super]) }}" method="POST" class="grid gap-5">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button type="submit" class="inline-flex items-center rounded-full bg-brand-600 px-5 py-3 text-sm font-semibold text-white shadow-soft transition hover:-translate-y-0.5 hover:bg-brand-700">{{ __('precios.edit.submit') }}</button>
