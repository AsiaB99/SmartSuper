<x-guest-layout>
    <section class="w-full max-w-[420px] rounded-[20px] bg-white/95 p-10 shadow-[0_15px_35px_rgba(0,0,0,0.20)] backdrop-blur">
        ...
        <div class="mb-8 grid grid-cols-2 rounded-[12px] border border-[var(--color-borde-suave)] bg-white p-1 text-center text-sm font-semibold">
            <a href="{{ route('login') }}" class="rounded-[10px] bg-brand-500 px-4 py-3 text-white shadow-[0_4px_6px_rgba(46,204,113,0.20)]">Iniciar sesión</a>
            @if (Route::has('register'))
                <a href="{{ route('register') }}" class="rounded-[10px] px-4 py-3 text-ink-500 transition hover:bg-brand-50">Registrarse</a>
            @endif
        </div>
